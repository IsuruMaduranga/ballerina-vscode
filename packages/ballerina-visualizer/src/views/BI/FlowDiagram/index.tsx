...
export function BIFlowDiagram(props: BIFlowDiagramProps) {
    ...
    useEffect(() => {
        ...
        if (!model || sidePanelView !== SidePanelView.FORM || !openNode?.codedata) {
            return;
        }
        // New-node templates carry no lineRange of their own (the insertion point lives in
        // targetRef), so this branch must come before the lineRange guard below.
        if (openNode.codedata.isNew) {
            reanchorNewNodeTarget(openNode);
            return;
        }
        if (!openNode.codedata.lineRange?.startLine) {
            return;
        }
    ...
    const reanchorNewNodeTarget = (openNode: FlowNode) => {
        ...
        // Generic templates carry no lineRange (the target ref owns the insertion point);
        // the agent flows stamp one, which must shift with the target.
        const openLineRange = openNode.codedata.lineRange;
        if (openLineRange?.startLine) {
