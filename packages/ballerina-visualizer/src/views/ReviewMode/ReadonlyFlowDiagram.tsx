...
// The construct's before/after source when the diff carries it (added by the LS for function/
// resource diffs), or undefined. Lets the flow view fall back to a source diff when the LS can't
// produce a flow model. Derived in one place so the effect and the render branch stay in sync.
function getSourceFallback(
    expectedMetadata?: ExpectedFlowMetadata
): { name?: string; oldSource?: string; newSource?: string } | undefined {
    const meta = expectedMetadata?.metadata as { name?: string; oldSource?: string; newSource?: string } | undefined;
    return meta && (meta.oldSource !== undefined || meta.newSource !== undefined) ? meta : undefined;
}
...
export function ReadonlyFlowDiagram(props: ReadonlyFlowDiagramProps): JSX.Element {
    ...
    useEffect(() => {
        ...
        loadFlowModel()
            .then((model) => {
                ...
                if (!model) {
                    // The LS couldn't produce a flow model (timeout / generation failure). If the
                    // diff carries the construct's before/after source, fall back to a source diff
                    // so the change is never invisible; otherwise show a guidance message.
                    const fallback = getSourceFallback(expectedMetadata);
                    if (fallback) {
                        // OBJECT_FUNCTION covers both service resources and plain class methods;
                        // only resources carry an accessor. Labelling a method "Resource" would
                        // render an empty HTTP-method pill in the title bar.
                        const meta = expectedMetadata?.metadata as
                            | { accessor?: string; resourcePath?: string }
                            | undefined;
                        const isResourceConstruct =
                            expectedMetadata?.nodeKind === NODE_KIND_RESOURCE && !!meta?.accessor;
                        onModelLoadedRef.current?.({
                            type: isResourceConstruct ? "Resource" : "Function",
                            name: fallback.name ?? meta?.resourcePath ?? "",
                            accessor: isResourceConstruct ? meta?.accessor : undefined,
                        });
                    } else {
                        setUnavailableMessage(
                            "The diagram for this change couldn't be generated by the language server " +
                            "(it may have timed out or failed for this construct). Your change is applied — " +
                            "switch to the Code view in the chat to see the source diff."
                        );
                    }
                    return;
                }
    ...
    // A short construct label for the review-diff diagnostics log (e.g. "get /orders" or "main").
    const diagnosticLabel = ((): string | undefined => {
        const meta = expectedMetadata?.metadata as { name?: string; accessor?: string; resourcePath?: string } | undefined;
        if (meta?.accessor && meta?.resourcePath) {
            return `${meta.accessor.toLowerCase()} ${meta.resourcePath}`;
        }
        return meta?.name;
    })();

    const fetchVersion = (useFileSchema: boolean): Promise<Flow | null> =>
        fetchFlowModelVersion(rpcClient, modelCache, { filePath, position, oldPosition, useFileSchema, label: diagnosticLabel });
    ...
    if (!flowModel) {
        // Flow model unavailable: prefer the before/after source diff the change carries, so a
        // construct the LS can't diagram (e.g. an automation with connector calls) is still shown.
        const fallback = getSourceFallback(expectedMetadata);
        if (fallback) {
            return (
                <ReadonlySourceDiff
                    oldSource={fallback.oldSource}
                    newSource={fallback.newSource}
